...
scope to, and a paths-only mode for cheap breadth. `list_files` gains an optional
depth so a folder tree can be seen in one call instead of one call per level.
...
24. A model asking for `mkdir`, `mv` or `rename` is routed to the real tool
    rather than told it does not exist. `rm` and `cp` still say no.
25. `npm run lint` and `npm run build` pass.
26. An acceptance run against a small local model (the class the pre-pass exists
    for) still picks sensible tools with the larger tool list. This is the risk
    the change carries; design §11 says what happens if it fails.
