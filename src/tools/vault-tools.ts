...
import { ApprovalRequest, ApprovalResult, FileChange } from '../types';
import { activeToolSpecs } from './specs';
...
/**
 * What the plugin lends to anything that runs the agent, independent of which
 * run it is. Passed as one object because every caller needs the same five and
 * a positional list of them is a signature nobody can read — and because adding
 * a sixth should not be an arity change in three files.
 */
...
/**
 * Approvals the user granted for the rest of one conversation. Scoped to the
 * conversation rather than the plugin, which is why they travel beside the
 * dependencies instead of inside them.
 */
...
async function ensureWritable(
	...
	if (isWritable(p, ctx.settings) || ctx.grants.writes.has(p)) return null;
...
async function ensureMovable(ctx: ToolContext, from: string, to: string): Promise<string | null> {
	const ok = (p: string): boolean =>
		isWritable(p, ctx.settings) || ctx.grants.writes.has(p);
	...
	switch (decision) {
		case 'once':
			return null;
		case 'session':
			ctx.grants.writes.add(from);
			ctx.grants.writes.add(to);
			return null;
		case 'always-file':
...
async function ensureFolderAllowed(ctx: ToolContext, path: string): Promise<string | null> {
	if (isWritable(path, ctx.settings) || ctx.grants.writes.has(path)) return null;
...
async function callMcp(ctx: ToolContext, name: string, argsJson: string): Promise<string> {
	...
	if (!ctx.mcp.isTrusted(name) && !ctx.grants.mcp.has(name)) {
		const decision = await ctx.requestApproval({
			kind: 'mcp',
			tool: name,
			serverId: cfg.id,
