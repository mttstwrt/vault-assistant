...
A saved conversation records what the agent did as well as what it said, so a transcript can contain the text a tool returned — which means passages of notes the agent read can end up quoted inside the conversations folder. That is your vault either way and nothing leaves your machine because of it, but it is worth knowing if you share a transcript: a conversation about a note contains part of that note. Blocked folders are never read in the first place, so they cannot appear.
...
- **See every write as a diff** — when the agent writes a note, the panel shows what changed: green `+` lines, red `−` lines, a few lines of context, and the added/removed counts in the header. Within a changed line, **the part that actually changed is highlighted**, the way git and Obsidian's own diff mark it, so a one-word edit reads as a one-word edit rather than a whole line thrown away and rewritten. A replacement too dissimilar to pair is left plain — highlighting every token there would look like information without being any. Small changes are expanded, larger ones collapse behind their counts, and a wholesale replacement of a very large note is summarised rather than drawn. When a write needs your approval, the same diff appears **on the approval card**, so you decide on the actual change rather than just a path — and once you allow it, that card becomes the record of the write instead of a second copy of the diff. The diff is for you only: the model still just gets "Updated Notes/Ideas.md", so your context window doesn't pay for it.
...
- **Redo a turn, or take it another way** — every message carries its own actions. **Answer again** on the newest reply re-answers the standing question in place. Anything that would discard turns you might want — editing an earlier message, or re-answering an earlier reply — **forks instead**: the kept turns become a new transcript with `branched-from: [[the original]]` in its frontmatter, so the parent gains a backlink and the branch is a note you can read, search and link like any other. That is llama.cpp's conversation tree written in Obsidian's vocabulary: the vault's link graph *is* the tree, and no branch is hidden inside a database. A conversation that has never been saved has nothing to preserve, so it simply rewinds.
- **Find a conversation by what was said in it** — the history button searches transcript *names and contents*, showing the line it matched. Transcripts are named by a model, so what you remember is usually a phrase from inside one rather than the title it was given. A name match wins over a content match, and the text is only read once your query is long enough to be worth scanning for. **New chat** and **Open a previous conversation** are commands as well as buttons, so both can take a hotkey.
- **Saved conversations come back whole** — every chat is written to your conversations folder as a markdown note, named from the date and time plus a title the model picks after the first exchange (`2026-08-12 1432 Reworking the RAG chunker.md`), so the folder stays browsable. Reopen one from the panel's history button and you get the conversation back, not a summary of it: the thinking sections, every tool call **with what it returned**, the diff of every write, the approvals you answered and how, the errors, and what each turn cost. The model gets back the context it had, so carrying on works as though you had never closed it. It is still a note — the machinery is carried in HTML comments, which Obsidian renders as nothing, so reading one in the app reads as a conversation. New turns are appended to the same note. Turn **auto-save** off and nothing is written until you press the panel's **save** button — which saves the conversation you are in, and keeps saving that one as it grows. Turn **filing** on (off by default) and the same call that titles a conversation also says which folder under your conversations folder it belongs in, creating it if needed, so a year of chatting sorts by subject instead of only by date. The folders you already have are put in front of the model to reuse, which is what keeps a folder from filling up with `Coding`, `Programming` and `Dev`; conversations are filed once, when first saved, and never moved afterwards.
...
- **Semantic search (opt-in)** — index your vault with any OpenAI-compatible embedding model and give the agent fuzzy, meaning-based recall alongside literal search. Optionally index the wiki (so semantic search lands on curated pages) and past conversations (your messages and the agent's final answers only — tool output, reasoning, diffs and attached notes are never embedded, so a conversation cannot return the same passage twice under two names). The index lives in the plugin folder — never in your notes — updates incrementally as you edit, and strictly respects blocked folders.
